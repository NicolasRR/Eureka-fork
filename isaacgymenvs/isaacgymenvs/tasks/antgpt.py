...
class AntGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.velocity, self.ang_velocity, self.potentials, self.prev_potentials)
...
@torch.jit.script
def compute_reward(
    root_states: torch.Tensor, velocity: torch.Tensor, ang_velocity: torch.Tensor,
    potentials: torch.Tensor, prev_potentials: torch.Tensor
) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Extract the forward velocity of the ant, which is assumed to be along the x-axis
    forward_velocity = velocity[:, 0]
    
    # Reward for moving forward quickly
    scale_forward_reward = 1.0
    forward_reward = scale_forward_reward * forward_velocity

    # Potential-based reward: encourage progress towards a target
    potential_reward = potentials - prev_potentials

    # Transform the components for smooth, normalized rewards
    temp_forward_velocity = 0.1
    temp_potential = 0.05
    # Exponential transformation with temperature to normalize and smoothen rewards
    forward_reward_exp = torch.exp(forward_reward / temp_forward_velocity) - 1
    potential_reward_exp = torch.exp(potential_reward / temp_potential) - 1

    # Combined transformed reward
    total_reward = forward_reward_exp + potential_reward_exp

    # Create a dictionary to log each part of the reward
    reward_components = {
        'forward_reward': forward_reward,
        'potential_reward': potential_reward,
        'forward_reward_exp': forward_reward_exp,
        'potential_reward_exp': potential_reward_exp,
